...
function slideInPlinko(e) {
    $('#plinko-board').attr("src", "plinko/plinko.html");
...
function slideInBalloon(e) {
    $('#balloon-pop-board').attr("src", "balloon-pop/balloon-pop.html");
    removeBtnListeners(); // remove event listeners on game select buttons
...
function slideOutBalloon(e) {
    ...
    $('#balloon-pop-container').animate({left: 1000}, 1500, function() { // move plinko container off screen
        ...
        $('#balloon-pop-board').attr("src", "game-board-placeholder.html");
    });
...
function slideInBPW(e) {
    $('#prize-wheel-board').attr("src", "prize-wheel/prize-wheel.html");
    removeBtnListeners(); // remove event listeners on game select buttons
...
function slideOutBPW(e) {
    ...
    $BPWBack.off('click', slideOutBalloon); // remove event listener
...
$imgContent.each(function() {$(this).hide();}); // hide all of the images
$imgContent.eq(imgLength-1).show(); // show last image in the sequence
function imgRotater() { // after initial delay, last image will fade out, and first image will fade in
    var prev = imgIndex - 1;
    if (prev < 0) {
        prev = imgLength-1;
    }
    $imgContent.eq(prev).fadeOut(400);
    $imgContent.eq(imgIndex).fadeIn(400);
    imgIndex += 1;
    if (imgIndex >= imgLength) {
        imgIndex = 0; // reset index
    }
    setTimeout(imgRotater, imgRotaterDelay);
}
setTimeout(imgRotater, imgRotaterDelay);